...
const { adminRouter } = require('./routes/adminRouter'); // Import the admin router
const { studentRouter } = require('./routes/studentRouter'); // Import the student router

// Create an Express application instance
const app = express();
app.use(express.json()); // Enable parsing of JSON request bodies
app.use("/api/v1/faculty", facultyRouter) // Mount the faculty router at the specified path
app.use("/api/v1/admin", adminRouter)  // Mount the admin router at the specified path
app.use("/api/v1/student", studentRouter) // Mount the student router at the specified path
